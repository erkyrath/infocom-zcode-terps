	TITLE	"Apple ][ YZIP (c)Infocom","Display line Routine"
;
; some aux mem zero page stuff
;
DLVARS:
CHR_X	EQU	DLVARS		; (WORD) X pos for char (0-559)
LINEOFF	EQU	CHR_X+2		; (BYTE) offset into SCRBUFF
ENDBIT	EQU	LINEOFF+1	; (BYTE) ending bit offset
SAVEX	EQU	ENDBIT+1	; (BYTE) save original X
...
LINE_HEIGHT EQU	SV_INV+1	; (BYTE) counter for height
LINE_X	EQU	LINE_HEIGHT+1	; (WORD) width of line counter
CH_BYTE	EQU	LINE_X+2	; (BYTE) small offset
DLVARSL	EQU	CH_BYTE-CHR_X+1
...
DOL1:
	...
	sta	LINEOFF		; which byte in line on screen
	sta	LINE_X+LO	; X pos of line
	sta	LINE_X+HI	; part 2
	sta	SV_UND		; start underlining bit at zero

	lda	SV_INV		; get starting INV state
	sta	INVFLG		; okay!

	dec	LINEOFF		; inclusive count
...
DATAL:
	...
	ora	SV_UND		; pick up underline flag
...
STCH:
	...
	inc	LINEOFF		; point to next one
	ldy	LINEOFF		; get current offset
...
GET_SCRCX:
	ldx	CHR_X+LO	; pick this back up
	stx	LINE_X		; just use this
	lsr	A		; /2
	ror	LINE_X		; pick up remainder
	lsr	A		; /4
	ror	LINE_X		; pick up remainder
	lda	LINE_X		; restore X pos
	sta	SCRCX		; got it
