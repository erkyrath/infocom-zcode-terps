	TITLE	"Apple ][ YZIP (c)Infocom","APPLE ][ SCREEN STUFF"
...
L1L:
	...
	lda	CLSLEFT		; where is left edge again?
	sta	SCRCX		; here!
CLSLOOP:
...
SCMOD2:	inc	SCROLLY		; inc/dec for next destination Y
	...
	ldx	SCLLINES	; scroll up or down?
...
;
; COPY_SCRL - copy from SCR_LINE onto the screen.  LINEOFF says how
;	many bytes in SCR_LINE to copy.  FPC are assumed to be pointing
...
COPY_SCRL1:
	...
	sta	PAGE2SW,X	; set first bank 
	lda	ENDBIT		; did we end evenly?
	bne	COPYL22		; nope
	inc	LINEOFF		; make LINEOFF be a counter
COPYL22:
	ldy	#0		; zero y

	lda	LINEOFF		; if only one, then handle real special
	bne	COPYL21		; nope
...
;
	...
	and	SCR_LINE	; get first data byte
...
FIXLOOP:
	lda	SCR_LINE,X	; get screen byte
	ora	(COLORP),Y	; or with color byte
	sta	SCR_LINE,X	; save it
	iny			; next color byte
	cpy	#4		; wrapped?
	bne	FIXLP1		; nope
	ldy	#0		; restart
FIXLP1:
	...
	cpx	LINEOFF		; done yet?
	bcc	FIXLOOP		; nope
	beq	FIXLOOP		; still nope
	rts			; now we are done
;
...
MSWCNT:	db	1
MSSTATE: db	2		; 1 = turn off, 2= move
...
MSCON2:
	...
	jsr	GET_NYBBLE	; save nybble
 	ldx	MSCCNT		; get counter
	sta	SVCURS,X	; get saved data
	lda	MSCOLOR,X	; get cursor color
