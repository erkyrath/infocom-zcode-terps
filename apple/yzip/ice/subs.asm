	TITLE	"Apple ][ YZIP (c)Infocom","OPCODE SUPPORT SUBROUTINES"
...
;
GETVRG:
	jsr	GVCALC	; GET ADDRESS OF GLOBAL INTO [FPC]
	jsr	FETCHB	; and get hi part
	sta	VALUE+HI	
	jsr	NEXTFPC	; and point to next part
	jsr	FETCHB	; and go get it
...
;
PUTVLG:
	JSR	GVCALC		; put address in [FPC]
	lda	FPCBNK		; and move it to stash spot
	sta	SPCBNK
	lda	FPCH
	sta	SPCH
	lda	FPCL
	...
	jsr	NEXTSPC		; point to next byte
...
; EXIT: ABSOLUTE ADDRESS OF GLOBAL VAR IN [FPC]
GVCALC:
	sec		; time for a subtract
	...
	sta	FPCL	; ADDRESS OF THE
	lda	I+HI	; DESIRED GLOBAL VARIABLE
	adc	GLOBAL+HI	; STORE ADDRESS BACK IN [FPC]
	jsr	SETPC	; now get memory page and bank
	sta	FPCH	; hi part
	sty	FPCBNK	; and bank
WCEX:
	rts
...
REFRSH:	LDA	ZBEGIN+ZFLAGS+1	; PICK UP LOW BYTE OF FLAG WORD
	ORA	#%00000100	; SET BIT 2
	STA	ZBEGIN+ZFLAGS+1	; AND PUT IT BACK
	RTS
