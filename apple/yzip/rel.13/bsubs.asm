...
ENDM:	DB	EOL,"End of session."
	DB	EOL,"Please insert new disk to boot."
ENDML	EQU	$-ENDM
...
SETPC:			
	...
	lda	#>ZBEGIN 	; ADD OFFSET TO GET RAM PAGE
...
NEXTFPC:
	...
	cmp	#>PRGLBL	; have we reached end of line?
...
DO_DSEGS:
	...
	lda	#>(SEGTBL+SGTCHKS1)	; start off at beginning
	sta	D1SEG+HI	; here is first one
	lda	#<(SEGTBL+SGTCHKS1); and here is LSB
...
;
; DO_PREFIX - check out the program name at $280.  If there is a prefix
;       in there, just set prefix to it, so my default prefix is just fine.
;
DO_PREFIX:
        ldx     START_NAME              ; get length of name
DOPLOOP:
        lda     START_NAME,x            ; get char
        and     #$7F                    ; make it real ASCII
        cmp     #'/'                    ; directory seperator?
        beq     DOPLX                   ; yup, found the prefix
        dex                             ; previous char
        bne     DOPLOOP                 ; go do it
        rts                             ; prefix is just fine as it is
DOPLX:
        stx     START_NAME              ; just reuse the name
        SET_PREFIX START_PREFIX         ; and go set it to this one
        rts
START_PREFIX:
        DB      1                       ; just one parm        
        DW      START_NAME              ; use my start up name
;
; INIT_CHARSET - move the default charset into lower memory
;
ICHARS:
        db      "abcdefghijklmnopqrstuvwxyz"    ; CHARSET 1
        db      "ABCDEFGHIJKLMNOPQRSTUVWXYZ"    ; CHARSET 2
        db      0,EOL,"0123456789.,!?_#'""/\-:()" ; CHARSET 3
ICHARSL EQU     $-ICHARS-1

INIT_CHARSET:
        lda     ZBEGIN+ZCHRSET          ; see if game has passed one
        ora     ZBEGIN+ZCHRSET+1        ; well?
        bne     INITCHG                 ; yes, so use that one
        ldx     #ICHARSL
INITCL:
        lda     ICHARS,X                ; get char
        sta     CHARSET,X               ; save char
        dex                             ; next char
        bpl     INITCL                  ; go get it
        rts
INITCHG:
        lda     ZBEGIN+ZCHRSET          ; get game page
        jsr     SETPC                   ; find out where in memory it be
        sta     FPCH                    ; save page
        sty     FPCBNK                  ; and save the bank
        lda     ZBEGIN+ZCHRSET+1        ; get the LSB
        sta     FPCL                    ; just save it
        lda     #78                     ; 78 characters are in it
        sta     J                       ; use J as counter
        lda     #0                      ; set up index
        sta     K                       ; in K
INCGL:
        jsr     FETCHB                  ; go get the character
        ldx     K                       ; this is index
        sta     CHARSET,X               ; save game character
        jsr     NEXTFPC                 ; point to next character
        inc     K                       ; point to next storage byte
        dec     J                       ; count character
        bne     INCGL                   ; go get next char

        lda     #EOL                    ; make sure EOL is in list
        ldx     #53                     ; offset of 53
        sta     CHARSET,X               ; and away it goes
