...
ZLOC:
	lda	ARG1+LO	
	ldx	ARG1+HI	
	cpx	#$20	; don't let it get too high
	bcc	ZLOC1	; fine
	lda	#22	; too big object
	jmp	ZERROR	; and die
ZLOC1:
...
SAVEOBJ: ds     1       ; check for zero object
...
REMVC1:
	...
	jsr	FETCHB
        sta     SAVEOBJ ; save for later check
	tax		; and put into x for (possible) check
	pla		; get lo part back
        tay             ; save a minute
        ora     SAVEOBJ ; well, is it zero?
        bne     REMVC2  ; no, so check it
        lda     #21     ; so die
...
ZTHROW:
	lda	ARG2+LO		; check to see if we are ready to do return
	cmp	OLDZSP+LO
	bne	ZTHLOOP		; nope, keep unwinding the stack
	lda	ARG2+HI
	cmp	OLDZSP+HI	; have we found the right frame yet?
	beq	ZTHDO		; yes
ZTHLOOP:
	jsr	ZRET		; do another return
	jmp	ZTHROW		; and try again
ZTHDO:	
	lda	ARG2+LO		; SET ZSTACK POINTER
