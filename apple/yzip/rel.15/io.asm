...
TBLRTN:
	...
	jsr	NEXTSPC		; point past the (word) counter
...
TBLSPX:
	...
	jsr	NEXTSPC		; point to LSB of next count
	lda	#0		; and stash another zero there
	jsr	STASHB		; away it goes
	jsr	FP2SP		; have SPC point to beginning
	lda	#0		; upper byte of count is zero
	jsr	STASHB		; show it
	jsr	NEXTSPC		; point to LSB
	lda	J		; get how many chars are there	
	jsr	STASHB		; save at beginning of line
	inc	J		; count (word) counter in offset
...
FLUSH:
	txa			; is it zero?
	bne	FL00		; nope
	rts			; just ignore if only one character
FL00:
...
LINOUT:
	...
	bne	LOUT0		; yes, so do it
	lda	FMTTBL		; formatted table?
	beq	LOUT1		; check scripting
	jmp	TBLRTN		; flush to table
LOUT0:
...
;
; not formatted table, so put count into table
;
...
RESET0:
	...
	jmp	STASHB		; and save it, and done
DRESFT:
...
START_TABLE:
	...
	sta	DIRTBL+LO	
	lda	#2
	sta	DIRITM+LO	; start at second char
	lda	#0		; zero item counter
	sta	DIRITM+HI	
...
ZPFL1:
	jsr	GETBYT		; get the length byte, MSB, & ignore
...
INP_X:	dw	0		; saved cursor position
...
INPUT:
	...
	lda	CURSFLG		; really move the cursor?
	beq	IO_CURS		; nope
;
; re-doing the (hopefully) saved read, so put cursor back
;
	lda	INP_X+HI	; first, the MSB
	sta	CHR_X+HI
	lda	INP_X+LO	; now the LSB
	sta	CHR_X+LO
...
;
; now, save CHR_X, in case we redo this read
;
	lda	CHR_X+HI	; first, the MSB
	sta	INP_X+HI
	lda	CHR_X+LO	; now the LSB
	sta	INP_X+LO
	rts			; Length is in [A]
;
