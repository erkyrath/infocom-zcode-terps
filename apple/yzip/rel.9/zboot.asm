	PAGE	
	STTL "--- ZIP CODE BOOT PART ---"
ZBOOT:
;
; clear out ZERO page stuff
;
	lda	ARG2+LO		; get machine id!
	sta	ZBEGIN+ZINTWD	; save before it gets zeroed
...
MSLOOP:
	DLINE	MMSG		; ask about Mouse/Joystick/Keyboard
        lda     #1              ; move to left
        sta     CHZ             ; move to column 1
	jsr	MBASCAL		; move cursor there!	
        lda     #EOL.OR.$80        ; send out eol
        jsr     MCOUT           ; bye
...
ZBEND:
;
; now, set flags like I want them
        lda     #FMONO^FDISP^FCOLO
        ldx     MOUSEF                  ; doing mousey stuff?
        beq     ZBEND0                  ; nope
        ora     #FMOUS                  ; then turn on that bit too
ZBEND0:
	sta	ZBEGIN+ZFLAGS+1	        ; set game flags
;
;
...
;
	...
	inc	TBLPUR		; so point to it
        lda     TBLPUR          ; does table preload take up all preload?
        cmp     #PGBEGIN        ; reach 1st paged page?
        bcs     ZBEND1          ; ayyup, no function preload
...
ZBEND1:
	...
	sta	MSTBL+HI	; and page
        ...
        jsr     INIT_CHARSET    ; initialize the charset
