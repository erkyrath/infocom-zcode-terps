	TITLE	ZIP	Z-LANGUAGE INTERPRETER      HP-DOS 2.0 VERSION

	.XLIST

	; PATCHES AND FIXES
	;
	; 1) Random bug fixed - 06/25/84 - Dan
	; 2) Status Line Bug (Zork II) Fixed - 07/24/84 - Dan
	; 3) Status line bug fixed (restart) 17-Oct-84/Paul
...
DATA_SG	SEGMENT	PARA
	...
	;GTBLKS
GAMFCB	DB	0,"SAMPLER DAT",30 DUP(0)
SAVFCB	DB	0,"SAMPLER SAV",30 DUP(0)
DSKFCB	DW	0
...
SCPL	DB	80	; COLUMNS PER SCREEN
SLPP	DB	24	; LINES PER SCREEN

	; STANDARD ANSI ESCAPE SEQUENCES FOR:
	;
	; 1) ERASE SCREEN/ALL ATRIB. OFF/POSITION CURSOR 1,1
	; 2) INVERSE VIDEO ON/POSITION 1,1
	; 3) ATRIB. OFF/ POSITION BOTTOM,1
	;
	
STINIT	DB	16
	DB	27,"[2J"	; CLEAR SCREEN
	DB	27,"[0m"	; ALL ATTRIBUTES OFF
	DB	27,"[01;01H"	; POSITION CURSOR 1,1

STRESET	DB	0

SBLINE	DB	12
	DB	27,"[7m"	; INVERSE VIDEO ON
	DB	27,"[01;01H"	; POSITION CURSOR 1,1

SELINE	DB	12
	DB	27,"[0m"	; ALL ATTRIBUTES OFF
	DB	27,"[16;01H"	; POSITION CURSOR 16,1

SPINIT	DB	0

	; HP/150 SCREEN DEFINITIONS
	; ESCAPE CODES FOR HP 150
	; FOR SAME AS ABOVE

HPTYPE	DB	1	; 0=110 , 1=150 ASSUME 150 FOR NOW

HPTINIT	DB	17	; LENGTH OF STRING
	DB	27,"H"	; HOME CURSOR
	DB	27,"J"	; CLEAR TO END OF MEMORY
	DB	27,"&d@"	; TURN OFF ANY ATTRIBUTES
	DB	27,"&a00c00Y"	; POSITION CURSOR AT 1,1

HPBLINE	DB	13	; LENGTH OF STRING
	DB	27,"&a00c00Y"	; POSITION CURSOR AT 0,0
	DB	27,"&dB"	; TURN ON INVERSE VIDEO

HPELINE	DB	13	; LENGTH OF STRING
	DB	27,"&a00c24Y"	; POSITION CURSOR AT 1,23
	DB	27,"&d@"	; TURN OFF ANY ATTRIBUTES

TERMID	DB	15	; LENGTH OF STRING
	DB	27,"&s1G"	; SET STRAP (G)
	DB	27,"&s1H"	; SET STRAP (H)
	DB	27,"*s1^"	; SEND TERMINAL ID
...
RSEED2	DW	?		;SEED NUMBER TWO
RTEMP	DW	?		;TEMP FOR RANDOM
...
IDSTR	DB	"HP-DOS 2.0 Interpreter Version D",0
...
STR15$:	MOVM	[BX],DS:[BP],AL	;TRANSFER THEM
	...
	MOV	BUFPGS,23	;OUR TABLE IS ONLY SO BIG....
	MOV	SI,23
STR16$:	SHL	SI,1
...
RESTRT	PROC
	...
	MOV	CX,PURBOT
	CALL	GTBLKS
	JMP	RESTRT1		; PATCH TO FIX STATUS LINE (3)
START1:	
...
SYSINI	PROC
	...
	MOV	BX,OFFSET TERMID	; FIND OUT WHICH COMPUTER
	CALL	MSPRT		; SEND IT TO SYSTEM

	MOV	AH,8		; CONSOLE INPUT WITHOUT ECHO
	INT	21H		; GET THIRD CHARACTER OF ID SEQ.
	INT	21H		; SHOULD BE "110A(CR)(LF)"
	INT	21H		;    OR     " 150A(CR)(LF)"
	CMP	AL,"5"		; IF IT'S (5) THEN IT'S A 150
	JE	AR2		; SET UP FOR 150
	CMP	AL,"2"		; 2623 CONFIGURATION
	JE	AR2
	
	MOV	SCPL,78		; NUMBER OF COLUMNS FOR HP/110
	MOV	SLPP,15		; NUMBER OF ROWS FOR HP/110
	JMP	DGONE

AR2:	INT	21H
	MOV	SCPL,78		; NUMBER OF COLUMNS FOR HP/150
	MOV	SLPP,23		; NUMBER OF ROWS FOR HP/150

DGONE:	MOV	AH,8
	INT	21H		; CLEAR OUT THE BUFFER
	INT	21H
	RET
...
GTBKIR:	MOV	DX,DSKFCB
	...
	CMP	AL,1		; 1=NO DATA 
	JE	GTB2
	CMP	AL,2		; 2=DISK FULL
	JE	GTB2		; 0=SUCCESSFUL , 3=PARTIAL READ 0 FILLED
	CMP	DSKDIR,0
	JNE	GTBKK
	PUSH	SI
	PUSH	DI
...
MSOUT	PROC
	CMP	HPTYPE,1	; IS IT A 150
	JNE	AR01		; NO

	MOV	BX,OFFSET HPBLINE
	CALL	MSPRT
	MOV	AH,09H
	MOV	DX,SLSTR
	INT	21H
	MOV	BX,OFFSET HPELINE
	CALL	MSPRT
	RET

AR01:	MOV	BX,OFFSET SBLINE
	CALL	MSPRT
	MOV	AH,09H
	MOV	DX,SLSTR
	INT	21H
...
MINIT1: CMP	HPTYPE,1	; IS IT A 150
	JNE	AR1		; NO THEN DO ANSI
	MOV	BX,OFFSET HPTINIT
	CALL	MSPRT
	MOV	BX,OFFSET HPELINE
	CALL MSPRT
	RET

AR1:	MOV	BX,OFFSET STINIT ; ANSI OUTPUT
	CALL	MSPRT
	MOV	BX,OFFSET SELINE
	CALL	MSPRT
	RET
